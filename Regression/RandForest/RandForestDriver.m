...
for i = 1:(numel(OutNames))
    disp(OutNames{i})
    OutName = OutNames{i};
    % Mdl = fitrensemble(In_train(:,Importances(i,1:20)),Out_train(:,i).Variables,...
...
function status = makeScatter(name,saveID,Predict,Actual,scores)
    ...
    title("Ensemble Regression: "+name, 'FontSize',20);
    set(gca,'FontSize',16);
    set(gca,'LineWidth',2);
